    ...
    def make_move(self, game: a2_minichess.MinichessGame, previous_move: Optional[str]) -> str:
        ...
        if self._game_tree is None or self._game_tree.get_subtrees() == []:
            ...
            if previous_move is None:
                ...
                var_2 = self._game_tree.find_subtree_by_move(previous_move)
                if var_2 is None:
                    self._game_tree = None
                    move = self.make_move(game, previous_move)
                else:
                    move = random.choice(var_2.get_subtrees()).move
                    var = var_2

        if self._game_tree is not None:
            sub = var.find_subtree_by_move(move)
            self._game_tree = sub
        return move
...
def part1_runner(games_file: str, n: int, black_random: bool) -> None:
    ...
    a2_minichess.run_games(n, white, black, False, True)
...
if __name__ == '__main__':
    ...
    # part1_runner('data/small_sample.csv', 50, True)
